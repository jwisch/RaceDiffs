...
#######Data set up
source("W:/ances/julie/ADRC/RaceDiffs_Functions.R") #Has bootstrapping function and z scoring function
source("W:/ances/julie/Data/StarkRequest/CommonFuncs.R")
source("W:/ances/julie/ADRC/RaceDiffs_MRIcleaning_DF15.R") #Getting MRI Data

source("W:/ances/julie/ADRC/RaceDiffs_Amyloidcleaning.R") #Getting centiloid data
source("W:/ances/julie/ADRC/RaceDiffs_Taucleaning.R") #Getting tau data
source("W:/ances/julie/ADRC/RaceDiffs_rsfccleaning.R") #Getting rsfc data
...
CreateTableOne(vars = listVars, data = Amyloid, factorVars = catVars, strata = c("race2", "CDRgreaterthan0"))
CreateTableOne(vars = listVars, data = tau.once, factorVars = catVars, strata = c("race2", "CDRgreaterthan0"))
CreateTableOne(vars = listVars, data = MRI.once, factorVars = catVars, strata = c("race2", "CDRgreaterthan0"))
CreateTableOne(vars = listVars, data = df.once, factorVars = catVars, strata = c("race2", "CDRgreaterthan0"))
...
#Getting effect size
6.59599 / sqrt(((2.78*sqrt(50))^2 + (2.78417*sqrt(372))^2)/2)
...
pAmyloid<-ggplot(Amyloid, aes(x=CDRgreaterthan0, y=PUP_fSUVR_rsf_TOT_CORTMEAN, fill=race2)) +  
  ...
  ylim(c(0.9, 3.4)) + scale_x_discrete(labels=c("0" = "Cognitively Normal", "1" = "CDR > 0"))+theme(legend.position = "bottom")
...
pA_corrected<-ggplot(Amyloid_corrected, aes(x=CDRgreaterthan0, y=Centiloid_corrected, fill=race2)) +  
  ...
  scale_x_discrete(labels=c("0" = "Cognitively Normal", "1" = "CDR > 0"))+theme(legend.position = "none") + ylim(c(-50, 110))
...
pTau<-ggplot(tau.once_alt, aes(x=CDRgreaterthan0, y=Tauopathy, fill=race2)) +  
  ...
  xlab("") + ylab("Mean Cortical Tau Accumulation") + ggtitle("AV-1451") + guides(fill=guide_legend(title="Race")) + 
   scale_x_discrete(labels=c("0" = "Cognitively Normal"))+theme(legend.position = "bottom")
...
pwr.t2n.test(n1 = 33, n2= 267, sig.level = 0.05, d = abs(CohenD))        
...
pT_corrected<-ggplot(Tau_corrected, aes(x=CDRgreaterthan0, y=Tau_corrected, fill=race2)) +  
  ...
  scale_x_discrete(labels=c("0" = "Cognitively Normal", "1" = "CDR > 0"))+theme(legend.position = "none") 
...
ADsigVol<-ggplot(MRI.once, aes(x=CDRgreaterthan0, y=ADsig, fill=race2)) +  
  ...
  ylim(c(-3, 3)) + scale_x_discrete(labels=c("0" = "Cognitively Normal", "1" = "CDR > 0"))
...
0.5365315/ sqrt(((0.1068457*sqrt(169))^2 + (0.1068457*sqrt(936))^2)/2) #CDR

0.3870393/ sqrt(((0.0504214*sqrt(169))^2 + (0.0504214*sqrt(936))^2)/2) #Race
...
pV_corrected<-ggplot(Vol_corrected, aes(x=CDRgreaterthan0, y=Vol_corrected, fill=race2)) +  
  ...
  scale_x_discrete(labels=c("0" = "Cognitively Normal", "1" = "CDR > 0"))+theme(legend.position = "bottom") 
...
0.02707591/ sqrt(((0.02130804*sqrt(71))^2 + (0.02130804*sqrt(432))^2)/2) #race

0.07090612/ sqrt(((0.03799188*sqrt(71))^2 + (0.03799188*sqrt(432))^2)/2) #CDR
...
13.27000/ sqrt(((12.24944*sqrt(80))^2 + (12.24944*sqrt(542))^2)/2)
23.14374/ sqrt(((2.77910*sqrt(80))^2 + (2.77910*sqrt(542))^2)/2)
1.17172/ sqrt(((0.13679*sqrt(80))^2 + (0.13679*sqrt(542))^2)/2)
...
mma_Vol<-mma(X,Y,pred=pred,contmed=c(1, 4:7),catmed=c(2),
          catref="male",binmed = c(3), binref = 0, jointm=list(n=1,j1=c(4:7)),
          predref="White",alpha=0.05,alpha2=0.05,n=20,
          n2=500,seed=457,nonlinear=T)


summary(mma_Vol, alpha = 0.05)
...
data.estimates = data.frame(
  ...
  mean = c(-0.310, -0.195, -0.115, -0.086, -0.001, 0.000, -0.032),
  se = c(0.074, 0.062, 0.040, 0.032, 0.004, 0.008, 0.025))
...
p1<-ggplot(data.estimates, aes(var, mean)) + 
  ...
  xlab("") + ylab("Estimated Effect") + ggtitle("Race - AD Signature Volume Mediation Analysis\nEffect Sizes") +
  theme(legend.position = "none") + ylim(c(-0.4, 0.1))


data.estimates = data.frame(
  var   = c('Total Effect', 'Direct Effect', 'Indirect Effect', 'ADI', 'Systolic BP', 'BMI', 'WMH'),
  mean = c(-0.021, 0.003, -0.025, -0.020, 0.00, -0.003, -0.002),
  se = c(0.017, 0.007, 0.014, 0.011, 0.002, 0.003, 0.007))
data.estimates$var <- factor(data.estimates$var, levels = c('Total Effect', 'Direct Effect', 'Indirect Effect', 'ADI', 'WMH', 'Systolic BP', 'BMI'))
data.estimates$CI_low<-data.estimates$mean - data.estimates$se
data.estimates$CI_high<-data.estimates$mean + data.estimates$se
p2<-ggplot(data.estimates, aes(var, mean)) + 
  geom_point(aes(x = var, y = mean, size = 7)) +
  geom_errorbar(aes(x = var, ymin = CI_low, ymax = CI_high), stat = "identity") + geom_hline(yintercept = 0, linetype = "dashed") +
  xlab("") + ylab("Estimated Effect") + ggtitle("Race - Intranetwork rsfc Signature Mediation Analysis\nEffect Sizes") +
...
Amyloid<-merge(Amyloid, ADI, by = "ID", all.x = TRUE, all.y = FALSE)
X<-Amyloid[,c("Age", "GENDER", "apoe4", "EDUC", "BPSYS", "BMI", "WMH_Volume", "CDRgreaterthan0")]
Y<-Amyloid[,c("PUP_fSUVR_rsf_TOT_CORTMEAN")]
pred<-Amyloid[,"race2"]
